    ...
    return (
      <Block className={"fixed space-x-4 w-full z-1 bg-[black] px-4 py-3"}>
        ...
        <Block className="text-white text-lg xl:my-8 lg:my-6 md:my-4 my-2  inline-flex justify-start items-start">
          <div className="mx-2"  ><VideoIcon /></div>
          <Grid className="text-[22px] font-extrabold">Upload Video</Grid>
        </Block>      </Block>
    );
  };
  ...
  const videoUpload = () => {
    ...
    if (validateFields()) {
      ...
      if (videoMetaData?.video && videoMetaData?.video instanceof File) {
        formData.append('videoFile', videoMetaData?.video);
      } else {
      ...
      if (videoMetaData?.image && videoMetaData?.image instanceof File) {
        formData.append('videoThumbnail', videoMetaData?.image);
      } else {
      ...
      // Add string fields
      formData.append('videoTitle', videoMetaData?.title || '');
      formData.append('videoDescription', videoMetaData?.description || '');
      formData.append('hashTag', videoMetaData?.hashTag || '');
      formData.append('videoType', String(videoMetaData?.videoType || ''));
  ...
  const handleVideoUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      video.onloadedmetadata = () => {
        ...
        // Cleanup the object URL after the metadata is loaded
